...
public class QueryParser {
	...
	static{
		// parserPluginsQuery配置于qparser_plugins_query.xml文件
		parserPlugins = (ParserPlugins) ApplicationContextHelper.getBean("parserPluginsQuery");
	}
		...
		try {
			...
			if (ret != null) {
				...
				if (result != null && result.length() > 0)
					return getJsonResult(query, type, "OK", 
							""/*ret.processLog.replace("\"", "'").replace("\n", "<br>")*/,
							result);
				else
					return getJsonResult(query, type, "NO_PARSER", "", "");
			} else {
		...
		for (int i = 0; i < qlist.size(); i++) {
			if (qlist.get(i).size() > 0) {
				...
				if (listEnv.containsKey("jsonResult")) {
					if (sb.length() > 0)
						sb.append(",");
					sb.append(listEnv.get("jsonResult", String.class, false));
				}
			}
		}
		return sb.toString();
	...
	public static void main(String[] args) {
    	///*
    	System.out.println(QueryParser.queryParser("近期创阶段新高 前复权", ""));
    	System.out.println(QueryParser.queryParser("20130901 市盈率", ""));
    	System.out.println(QueryParser.queryParser("2013年9月 涨跌幅 前复权", ""));
    	System.out.println(QueryParser.queryParser("市盈率 市净率", ""));
    	System.out.println(QueryParser.queryParser("PE>10 收盘价>10元", ""));
    	System.out.println(QueryParser.queryParser("", ""));
    	System.out.println(QueryParser.queryParser("123", ""));
    	//*/
