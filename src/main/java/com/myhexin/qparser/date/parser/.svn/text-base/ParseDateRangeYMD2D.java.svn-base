...
public class ParseDateRangeYMD2D implements Parser
{
	public DateRange doParse(String dateString, String backtestTime) throws UnexpectedException,
			NotSupportedException
	{
		if (!DatePatterns.YMD_2_D_NO11.matcher(dateString).matches())
		{
			return null;
		}
		if (Param.DEBUG_DATEINFO)
		{
			System.out.println("YMD_2_D_NO11");
		}
		Matcher mid = DatePatterns.YMD_2_D_NO11.matcher(dateString);
		mid.matches();
		if (!NumUtil.checkAsYMD(mid.group(2), mid.group(3), mid.group(4))
				|| !NumUtil
						.checkAsYMD(mid.group(2), mid.group(3), mid.group(5)))
		{
		...
		DateInfoNode pointend = new DateInfoNode(Integer.valueOf(mid.group(2)),
				Integer.valueOf(mid.group(3)), Integer.valueOf(mid.group(5)));
