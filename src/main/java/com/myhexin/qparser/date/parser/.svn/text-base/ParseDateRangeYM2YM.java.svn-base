...
public class ParseDateRangeYM2YM implements Parser
{
	public DateRange doParse(String dateString, String backtestTime)
	{
		if (!DatePatterns.YM_2_YM_NO12.matcher(dateString).matches())
		{
			return null;
		}
		if (Param.DEBUG_DATEINFO)
		{
			System.out.println("YM_2_YM_NO12");
		}
		Matcher mid = DatePatterns.YM_2_YM_NO12.matcher(dateString);
		mid.matches();
		DateInfoNode pointhead = new DateInfoNode(
				Integer.valueOf(mid.group(2)), Integer.valueOf(mid.group(3)), 1);
		DateInfoNode pointend = new DateInfoNode(Integer.valueOf(mid.group(4)),
				Integer.valueOf(mid.group(5)), DateUtil.getMonthDayCount(
						Integer.valueOf(mid.group(4)),
						Integer.valueOf(mid.group(5))));
