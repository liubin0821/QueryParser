...
public class ParseDateAfterLength implements Parser
{
	public DateRange doParse(String dateString, String backtestTime) throws NotSupportedException
	{
		if (!DatePatterns.IN_ONE_DAY_AFTER_LENGTH.matcher(dateString).matches())
		{
			return null;
		}
		if (Param.DEBUG_DATEINFO)
		{
			System.out.println("IN_ONE_DAY_AFTER_LENGTH");
		}
		// 匹配某天之后，“2月以后”
		Matcher mid = DatePatterns.IN_ONE_DAY_AFTER_LENGTH.matcher(dateString);
		mid.matches();
		...
		try
		{
			to = DateUtil.getNewDate(from, Unit.DAY, -1);
