...
public class ParseSeqMonthWithSomething implements Parser
{
	public DateRange doParse(String dateString, String backtestTime) throws UnexpectedException,
			NotSupportedException
	{
		if (!DatePatterns.SEQ_MONTH_WITH_SOMETHING.matcher(dateString)
				.matches())
		{
			return null;
		}
		...
		{
			System.out.println("SEQ_MONTH_WITH_SOMETHING");
		}
		Matcher mid = DatePatterns.SEQ_MONTH_WITH_SOMETHING.matcher(dateString);
		mid.matches();
		...
		ArrayList<DateRange> splitDates = DateUtil.getSplitDateByOtherDay(
				splitYears, quarterRange, Unit.DAY);
