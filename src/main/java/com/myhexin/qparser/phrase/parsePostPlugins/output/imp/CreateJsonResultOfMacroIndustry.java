...
public class CreateJsonResultOfMacroIndustry extends Output {
	protected static org.slf4j.Logger logger_ = org.slf4j.LoggerFactory.getLogger(CreateJsonResultOfMacroIndustry.class.getName());
	private static TemplateResultBuilder templateBuilder = new TemplateResultBuilder(Param.MACRO_INDUSTRY_PARSER_TEMPLATES_PATH);
	...
	static {
		...
		cfg_.setClassForTemplateLoading(Thread.currentThread().getClass(), Param.MACRO_INDUSTRY_PARSER_TEMPLATES_PATH);
	}*/
	
	public static String getJsonResult(ArrayList<SemanticNode> nodes) {
		List<String> indexConds = getIndexConds(nodes);
		List<String> filtConds = getFiltConds(nodes);
		if ((indexConds != null && indexConds.size() > 0)
				&& (filtConds != null && filtConds.size() > 0))
			return createTemplate("ifind", indexConds.get(0) + "," + filtConds.get(0));
		else if ((indexConds != null && indexConds.size() > 0)
				&& (filtConds == null || filtConds.size() == 0))
			return createTemplate("ifind", indexConds.get(0));
		else
			return null;
	}
	
	public static List<String> getIndexConds(List<SemanticNode> nodes) {
		if (nodes == null || nodes.size() == 0)
    		return null;
    	StringBuffer buffer = new StringBuffer();
        Iterator<?> iterator = new IndexIteratorImpl(nodes);
        if (!iterator.hasNext())
			return null;
        ArrayList<SemanticNode> indexs = new ArrayList<SemanticNode>();
    	while (iterator.hasNext()) {
    		indexs.add((SemanticNode) iterator.next());
    	}
    	buffer.append(createTemplate(indexs, "indexConds"));
    ...
    public static List<String> getFiltConds(List<SemanticNode> nodes) {
    	if (nodes == null || nodes.size() == 0)
    		return null;
        ...
        if (matchSub == 0) {
        	...
        	for (StringBuffer buffer : buffers)
        		strs.add(createTemplate("filtConds", buffer.toString()));
        } else if (matchSub > 1 && templateSub > 1) {
        	for (StringBuffer buffer : buffers)
        		strs.add(createTemplate("filtConds", buffer.toString()));
        }
    ...
    public static String createTemplate(List<SemanticNode> arguments, String semanticPatternId) {
    	if (arguments == null || arguments.size() == 0 
    			|| semanticPatternId == null || semanticPatternId.length() == 0)
    		return "";
    	// 第二步
		Map<String, List<SemanticNode>> root = new HashMap<String, List<SemanticNode>>();
		root.put("args", arguments);
		...
		/*try {
			if(notExistsTempleteSet.contains(templateFileName)) return "";
    ...
    public static String createTemplateByPatternId(ArrayList<ArrayList<SemanticNode>> arguments, String semanticPatternId) {
		...
		{
			ArrayList<SemanticNode> arg = arguments.get(i);
			if(arg!=null && arg.size()>0)
			{
				root.put("arg"+(j+1), arguments.get(i));
				j++;
			}
		}
		...
		if (semanticPatternId != null && !semanticPatternId.equals(""))
			templateFileName = semanticPatternId.toLowerCase()+".ftl";
		if(templateFileName.equals("165.ftl") ){ //"165.ftl"这个模板必须满足条件有2个args
			if(j>=2){
				ArrayList<SemanticNode> arg2 = (ArrayList<SemanticNode>)root.get("arg2");
				if(arg2!=null && arg2.size()>0 && arg2.get(0).isNumNode()) {
					return templateBuilder.createTemplate(root, templateFileName);
				}
			}
			return null;
    ...
    public static String createTemplate(String logic, String operands) {
		...
		String templateFileName = logic.toLowerCase()+".ftl";
		//return createTemplate(templateFileName, root);
		return templateBuilder.createTemplate(root, templateFileName);
    }
    
    /*public static String createTemplate(String templateFileName, Map root) {
    	try {
    		if(notExistsTempleteSet.contains(templateFileName)) return "";
			Template t = cfg_.getTemplate(templateFileName);
			Writer out = new StringWriter();
    	...
    	if (patternId.equals(IMPLICIT_PATTERN.KEY_VALUE.toString())) {
			...
			for (int j = 1; (elelist = info.getElementNodePosList(j)) != null; j++) {
				for (int pos : elelist) {
					...
					if(cn==null || (cn.isBoundToIndex() && !cn.isBoundValueToThis()))
						continue;
