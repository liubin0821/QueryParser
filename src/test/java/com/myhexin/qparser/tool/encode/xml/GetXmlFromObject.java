 ...
 */
public class GetXmlFromObject {
	private static final org.slf4j.Logger logger_ = org.slf4j.LoggerFactory.getLogger(GetXmlFromObject.class.getName());   
	...
	private int setId = 1;
	private String[] ignoreClass={"org.slf4j.Logger"};
	private List<String> ignoreClassList= Arrays.asList(ignoreClass);
	private boolean isInputObject = true;
	private String inPutObjName = "object1";
	...
	private final ArrayList<Field> ignoreSomeField(ArrayList<Field> fields){
		ArrayList<Field> tempFields = new ArrayList<Field>();
		for (Field field : fields) {
			//被忽略的不记录
			if ((field.getModifiers() & Modifier.STATIC) == Modifier.STATIC
					&& ignoreClassList.contains(field.getType().getName()))
				continue;
			//static final类型不记录
			if ((field.getModifiers() & Modifier.STATIC) == Modifier.STATIC
	...
	private String createXMLBean(Element root, Object o) {
		...
		ArrayList<Field> fields = ReflectionUtils.getDeclaredFields(o);
		
		
		//排除忽略的类型	
		fields=ignoreSomeField(fields);
		...
		for (int i = 0, len = fields.size(); i < len; i++) {
			...
			try {
				...
				} else if(o1.getClass().getSuperclass() == java.util.EnumSet.class){
					EnumSet es = (EnumSet) o1;
					property.addAttribute("class", t.getName());
					boolean done = false;
					for(Object obj : es){
						if(!done){
							done=true;
							property.addAttribute("enumClass", obj.getClass().getName());							
							property.setText(((Enum) obj).name());
							continue;
						}
						property.setText(property.getText()+","+((Enum) obj).name());
					}
	...
	private final boolean isEndType(Class<?> t) {
	...
	public GetXmlFromObject(String fileName) {
		super();
		if (fileName != null && !fileName.equals("")) {
			fileName_ = fileName;
		}
		else {
			logger_.error("文件名不能为空");
		}

	}
	
	public void createXML(Object o,String ObjctName) {
		inPutObjName = ObjctName;
		createXML(0);
	}
	 ...
	 * 
	 * @author: 	    吴永行 
	 * @dateTime:	  2013-12-27 上午9:11:09
	 * @description:  传入一个文件名,自动判断前面路径是否存在,不存在,自动创建
	 * @param 
	 *
	 */
	public static final void mkfiledirIfNotExist(String fileName){
		if(fileName==null || fileName.equals("")) 
			return;		
		//创建文件夹
		...
		if((pos=fileName.lastIndexOf(File.separator)) == -1)
			return;
		String dircect = fileName.substring(0,pos);
		if (!dircect.equals("")) {
			File file = new File(dircect);
