...
public class TestDetailCheckTwo extends TestCase {
	...
	public void test() {
		...
		try {
			...
			bw.write("query,checkRet,score,index,syntacticSemanticIds,newRet,error");
			bw.newLine();
			int num = 0;
	...
	private void saveCsv(BufferedWriter bw, ParseResult parseRet, String query,
			String check, String checkScore, String checkIndex, String checkSIds) {
		...
		try {
			String ret = parseRet.getStandardQueries().get(0);
			ret = ret.replace(";", "_&_");
			ret = ret.replace(",", ";");
			query = query.replace(",", ";");
			check = check.replace(",", ";");
			...
			if (score < 80) {
				...
				bw.newLine();
			} else if (!syntacticSemanticIds.equals(checkSIds)) {
